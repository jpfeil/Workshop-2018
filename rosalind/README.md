...
The first part of this workshop was to introduce you to useful python programming tools. Today, I want to show you some basic engineering skills for getting your project off the ground. We will be learning to use the very powerful IDE called Pycharm. After we begin
to write our rosalind problem solving project, we will learn how to track changes to the
project using git and github. Finally, we will discuss the very useful docker container
approach for packaging your project and distributing it.
...
### Argparse
Most bioinformatics software is used using the command line. The command-line gives you a lot of power to solve computational problems. If your command-line program takes off and does really well then you should think about making a graphical user interface, but until then you will likely write command-line tools.
...
your project is able to solve. In this case it is just echo, which isn't really
...
### Rosalind
In case you don't have enough problems to solve in your research, you can always practice bioinformatics using the Rosalind website. This is a great way to learn new algorithms and/or programming languages. Basically, Rosalind provides a series of questions with increasing difficulty. Each problem gives background for the biology and the computer science. It also provides a training data set with solution before allowing you to download the test data. Be aware that you only have 5 minutes to upload your solution to the problem.

#### Exercise:
We will make your program capable of solving all of the rosalind problems...eventually. We will use the subprogram feature of the argparse tool to determine which problem to solve. You have already done this by creating your argparse problem argument. Now you will need to add an additional problem to solve in your main. I recommend using the rosalind problem IDs, so DNA would be the first problem. After your if args.problem == 'echo', add a elif args.problem == 'DNA' and then create a function that solves that problem. You will also
...
Once your util.py module starts filling up, you will most likely want to separate them into
other modules that are organized based on the properties of these functions. So, you may
want a module for parsers, for sorting algorithms, for alignment algorithms, etc.
...
workshop, so don't worry about breaking this repo, but take care for future collaborative
projects.

### Workflow
Now I'm going to describe a workflow you can use to get work done. I have found that this is a nice way to collaborate with others on a project and keep track of progress. The basic idea is to make github issues for each modification to the repo and then make a git branch for each issue. Then, when you are done with your changes and you are ready to merge them with the master branch, you start a pull request that will merge those changes in.

#### Exercise:
1. Make a github issue on your fork for the next rosalind problem
2. Make a branch for the next rosalind probem using `git branch <name of branch>` (I recommend using the issue name in the branch name (i.e. issue-2/solve-rosalind-RNA)
...
#### Exercises:
Repeat this process again for the next rosalind problem. Once you merge the code into
